...
function showRoomingLoading(){
  const g=document.getElementById('roomingGrid'); 
  const l=document.getElementById('namelistContainer');
  ...
  const spinnerBilik = `<div class="col-span-2 flex flex-col items-center justify-center gap-3 py-10"><div class="w-8 h-8 border-[3px] border-slate-200 border-t-[#7A0C2E] rounded-full animate-spin"></div><div class="text-[11px] text-slate-600 font-medium">Memuatkan bilik...</div></div>`;
  const spinnerList = `<div class="flex flex-col items-center justify-center gap-3 py-16"><div class="w-8 h-8 border-[3px] border-slate-200 border-t-[#7A0C2E] rounded-full animate-spin"></div><div class="text-[11px] text-slate-600 font-medium">Memuatkan senarai jemaah...</div></div>`;
  if(l) l.innerHTML = spinnerList;
  if(g) g.innerHTML = spinnerBilik;
  const overview=document.getElementById('roomingOverview');
  if(overview) overview.innerHTML = `<div class="flex items-center gap-2 text-[11px] opacity-70"><span class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></span> Memuatkan MEKAH...</div>`;
  console.log('V133 showRoomingLoading - both sides loading');
}
...
function dropJemaah(e,roomId){
  ...
  document.querySelectorAll('[draggable="true"]').forEach(el=>{ el.style.opacity='1'; el.classList.remove('dragging'); });
  window._draggedJemaahId = null;
  // V133: If this is a room reorder drag, ignore jemaah drop - but only if it's actually a room
  try{
    const roomDragId = e.dataTransfer.getData('text/room-id');
    if(roomDragId && roomDragId.startsWith('rec') && allRoomingRecords.some(r=>r.id===roomDragId)){
      console.log('dropJemaah ignored - this is room reorder drag', roomDragId);
      return;
    }
    // Only treat plain as room reorder if draggedRoomId is active AND plain is a room ID (not jemaah)
    const plainCheck = e.dataTransfer.getData('text/plain');
    if(plainCheck && plainCheck.startsWith('rec') && allRoomingRecords.some(r=>r.id===plainCheck) && (draggedRoomId || window.draggedRoomId || window._draggedRoomId?.startsWith && allRoomingRecords.some(r=>r.id===window._draggedRoomId))){
      // Actually check if window._draggedJemaahId is set - if jemaah drag active, don't return
      if(!window._draggedJemaahId && (draggedRoomId || window.draggedRoomId)){
        console.log('dropJemaah ignored - plain is room id and room drag active', plainCheck);
        return;
      }
    }
  }catch(err){ console.warn('dropJemaah room check fail', err); }
