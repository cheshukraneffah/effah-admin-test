console.log('ROOMING V117 SIMPLE ALERT AUTO SET FIX SYNTAX - CAPAIAN LANGSUNG + TAPISAN + FIX SYNTAX');
...
var roomingFieldDefaults = {}; // Direct fetch only - no hardcoded
...
function renderRoomingHijriTabs(){
  ...
  if(seasons.length===0){
    container.innerHTML='<span class="text-[10px] text-slate-400">Tiada musim hijri ditemui</span>';
    return;
  }
...
async function fetchRoomingFieldOptionsFromMeta(){
  if(_roomingMetaFetching) return null;
  // OPTIMIZED: cache meta 10 minit, jangan fetch setiap kali
  // kalau cache masih valid <10min, guna cache
  if(_roomingMetaCache && (Date.now() - (_roomingMetaCache._time||0)) < 600000){
    console.log('META CACHE digunakan');
    _roomingMetaFetching=false;
    return _roomingMetaCache;
  }
  _roomingMetaFetching = true;
  try{
    const base = window.AIRTABLE_BASE_ID||localStorage.getItem('effah_api_base')||localStorage.getItem('effah_base_id');
    const pat = window.AIRTABLE_PAT||localStorage.getItem('effah_api_pat');
    if(!base||!pat){ 
      console.warn('V110: Base atau token PAT tidak ditemui untuk capaian metadata');
      _roomingMetaFetching=false; 
      return null; 
    }
    console.log('V110: Sedang mendapatkan metadata secara langsung...', base);
    const res = await fetch(`https://api.airtable.com/v0/meta/bases/${base}/tables`, {headers:{Authorization:`Bearer ${pat}`}});
    if(!res.ok){
      console.warn('V110: Gagal mendapatkan metadata Airtable', res.status, await res.text().then(t=>t.substring(0,500)));
      _roomingMetaFetching=false;
      // Fallback to records
    ...
    if(!targetTable){ 
      console.warn('V110: Jadual DATA JEMAAH UMRAH tidak ditemui dalam metadata');
      _roomingMetaFetching=false; 
      return null; 
    }
    ...
    wanted.forEach(w=>{
      ...
      if(f){
        ...
        console.warn(`V110: Medan ${w} tidak ditemui dalam metadata`);
      }
    });
    window.roomingFieldOptions=roomingFieldOptions;
    window._roomingFieldTypes=_roomingFieldTypes;
    window._roomingMetaCache=_roomingMetaCache;
    console.log('V110: Capaian langsung daripada metadata Airtable selesai', roomingFieldOptions, _roomingFieldTypes);
...
async function handleAddNewRoomingOption(fieldName, jemaahId=null){
  ...
  try{
    ...
    if(existingChoices.some(c=> (c.name||'').toUpperCase()===trimmed)){
      alert(`${trimmed} telah wujud di dalam Airtable.`);
      if(jemaahId){ try{ await updateJemaahField(jemaahId, fieldName, trimmed); }catch(e){} }
      return;
    }
    ...
    try{
      ...
      if(res.ok){ metaSuccess = true; } else { console.warn('V117 Meta failed', await res.text()); }
    }catch(e){ console.warn('V117 Meta exception', e); }
    if(jemaahId){
      ...
      if(rec){
        ...
        try{
          const finalPayload = rec.fields[fieldName];
          await fetch(`https://api.airtable.com/v0/${base}/DATA%20JEMAAH%20UMRAH/${jemaahId}`, {
            method:'PATCH',
            headers:{Authorization:`Bearer ${pat}`,'Content-Type':'application/json'},
            body: JSON.stringify({fields:{[fieldName]: finalPayload}, typecast:true})
          });
        }catch(e){ console.warn('V117 set value failed', e); }
      }
    } else {
    ...
    if(id.startsWith('pakejDrop-') || id.startsWith('visaDrop-') || id.startsWith('boardDrop-') || id.startsWith('insuranDrop-')){
      ...
      if(btn){
        ...
        setTimeout(()=>{
          ...
          if(tr.right>window.innerWidth) target.style.left=(window.innerWidth - tr.width - 10)+'px';
          if(tr.bottom>window.innerHeight) target.style.top=(rect.top - tr.height - 4)+'px';
        },0);
      }
    }
...
async function loadStaffList(){
  try{
    // Jika staffList sudah ada dari fetchRoomingData yang optimized, guna terus - jangan fetch lagi
    if(staffList && staffList.length>0 && _roomingFirstLoadDone){
      renderStaffList();
    ...
    // Guna filter juga untuk staff
    let allStaff=[];
    if(typeof fetchAirtableWithFilter==='function' && tripId){
      const filterFormula = `SEARCH("${tripId}", ARRAYJOIN({TRIP} & ""))`;
      allStaff = await fetchAirtableWithFilter('STAFF LIST (ROOMING)', filterFormula, 100);
    } else {
      // fallback lama
      let offset='';
      do{
    ...
    staffList=allStaff.map(r=>({
      id:r.id,
      airtableId:r.id||r.id,
      name:r.fields? (r.fields['NAME']||'') : (r.name||''),
      boardBasis:r.fields? (r.fields['BOARD BASIS']||'') : '',
      train:r.fields? !!r.fields['TRAIN'] : false,
      sortNumber:r.fields? (r.fields['SORT NUMBER']||9999) : 9999,
      trip:r.fields? (r.fields['TRIP']||[]) : [],
      roomIds: r.fields? (r.fields['ROOMING LIST'] || r.fields['ROOM'] || r.fields['BILIK'] || []) : (r.roomIds||[]),
      roomLink: r.fields? ((r.fields['ROOMING LIST']||[])[0]||null) : null
    }));
    // kalau allStaff sudah dalam format mapped (dari fetchAirtableWithFilter optimized path yang return mapped? actually returns raw, so above handles)
    // jika fetchAirtableWithFilter return raw, kita sudah map; jika return mapped dari fetchRoomingData path, staffList sudah set
    if(Array.isArray(allStaff) && allStaff.length>0 && allStaff[0].id && allStaff[0].name && !allStaff[0].fields){
      // sudah mapped dari fetchRoomingData
      staffList = allStaff;
    }
...
async function addNewStaff(){
  ...
  if(!name){ alert('Sila masukkan nama petugas.'); return; }
...
async function deleteStaff(staffId){
  if(!confirm('Adakah anda ingin memadam petugas ini daripada Senarai Extra?\n\nPetugas akan dikeluarkan daripada semua bilik sekiranya telah ditetapkan.')) return;
...
function openDeleteStaffModal(){
  ...
  if(staffList.length===0){ alert('Tiada petugas di dalam Senarai Extra.'); return; }
...
async function deleteSingleStaffFromModal(staffId){
  ...
  if(!confirm(`Adakah anda pasti untuk memadam ${name} daripada senarai Extra List?\n\nTindakan ini akan mengeluarkan petugas tersebut daripada semua bilik yang telah ditetapkan dan rekod akan dipadam daripada sistem.`)) return;
...
async function confirmBulkDeleteStaff(){
  ...
  if(checks.length===0){ alert('Sila pilih sekurang-kurangnya satu petugas untuk dipadam.'); return; }
  const ids = checks.map(c=>c.value);
  const names = ids.map(id=> staffList.find(x=>x.id===id||x.airtableId===id)?.name||id).join(', ');
  if(!confirm(`Adakah anda pasti untuk memadam ${checks.length} petugas berikut daripada senarai Extra List?\n\n${names}\n\nTindakan ini akan mengeluarkan kesemua petugas tersebut daripada semua bilik yang telah ditetapkan.`)) return;
  ...
  closeDeleteStaffModal();
  alert(`${ids.length} petugas telah dipadam.`);
}
...
async function deleteStaff(staffId){
  const s=staffList.find(x=>x.id===staffId||x.airtableId===staffId);
  const name = s?.name||'Staff ini';
  if(!confirm(`Adakah anda pasti untuk memadam ${name} daripada senarai Extra List?\n\nPetugas tersebut akan dikeluarkan daripada semua bilik yang telah ditetapkan.`)) return;
  await performDeleteStaff(staffId);
}
...
function renderInsuranCell(jId, insArr){
  ...
  html+='<div class="flex justify-between gap-1 mt-1 pt-1 border-t bg-white"><button onclick="clearInsuranMulti(\''+jId+'\'); closeInsuranDropdown(\''+jId+'\')" class="text-[9px] px-3 py-1 rounded-full bg-slate-100">Padam</button><button onclick="closeInsuranDropdown(\''+jId+'\')" class="text-[9px] px-3 py-1 rounded-full bg-[#7A0C2E] text-white">OK</button></div></div></div>';
  return html;
}
...
function showRoomingLoading(){
  ...
  const spinner = `<div class="flex flex-col items-center justify-center gap-3 py-10"><div class="w-8 h-8 border-[3px] border-slate-200 border-t-[#7A0C2E] rounded-full animate-spin"></div><div class="text-[11px] text-slate-600 font-medium">Sedang memuatkan data jemaah...</div></div>`;
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-2.5 flex flex-col gap-2">
      <div class="flex flex-wrap items-center justify-between gap-2">
        ...
        <div class="flex items-center gap-1.5 text-[11px]">
          <span id="belumAssignTop" class="px-2 py-0.5 bg-amber-100 rounded-full font-bold text-[10px]">0 Belum Ditetapkan</span>
        ...
        <div class="p-2.5 border-b border-slate-200">
          <div class="flex items-center justify-between mb-2.5">
            ...
            <div class="flex gap-1">
              <span id="belumAssignBadge" class="px-2 py-0.5 bg-amber-100 text-amber-800 rounded-full text-[10px] font-bold">0 Belum Ditetapkan</span>
          ...
          <div class="flex gap-1.5">
            ...
            <div class="relative">
              <button id="filterPakejRoomingBtn" onclick="toggleRoomingFilterBox()" class="text-[11px] border border-slate-200 rounded-xl px-3 py-2 bg-white font-bold flex items-center gap-1.5 hover:bg-slate-50">
                <i class="fa-solid fa-filter text-[10px]"></i> <span id="filterPakejRoomingLabel">All</span> <i class="fa-solid fa-chevron-down text-[9px]"></i>
              </button>
              <div id="roomingFilterBox" class="hidden absolute right-0 top-full mt-2 w-[340px] bg-white rounded-2xl shadow-xl border border-slate-200 z-[70] overflow-hidden">
                <div class="p-3 border-b border-slate-200 flex items-center justify-between">
                  <span class="font-bold text-[11px] tracking-wider">TAPISAN MENGIKUT MEDAN</span>
                  <button onclick="clearRoomingFilter()" class="text-[10px] px-2 py-1 bg-slate-100 rounded-full font-bold hover:bg-slate-200">Padam</button>
                </div>
                <div class="p-2 max-h-[50vh] overflow-y-auto space-y-3" id="roomingFilterBoxContent">
                  <div class="text-[11px] text-slate-400 text-center py-4">Sedang memuatkan tapisan...</div>
                </div>
              ...
              <select id="filterPakejRooming" class="hidden"><option value="">All</option></select>
            </div>
          </div>
        </div>
...
async function updateRoomCatatan(roomId, value){
  ...
  try{
    ...
    if(!base||!pat) throw new Error('Konfigurasi Airtable tidak ditemui. Sila semak tetapan API.');
  ...
  }catch(e){ console.error('Catatan update failed', e); alert('Gagal menyimpan catatan bilik: '+e.message+'\n\nField CATATAN BILIK tak wujud di Airtable? Check nama field.'); }
}
...
async function fetchAirtableWithFilter(tableName, filterFormula, pageSize=100, fields=null){
  const base=window.AIRTABLE_BASE_ID||localStorage.getItem('effah_api_base')||localStorage.getItem('effah_base_id'); 
  const pat=window.AIRTABLE_PAT||localStorage.getItem('effah_api_pat');
  if(!base||!pat) return [];
  let all=[], offset='';
  ...
  if(fields && fields.length>0){
    fieldsParam = fields.map(f=>`&fields%5B%5D=${encodeURIComponent(f)}`).join('');
  }
  do{
    let url = `https://api.airtable.com/v0/${base}/${encodedTable}?pageSize=${pageSize}${offset?`&offset=${offset}`:''}${fieldsParam}`;
    if(filterFormula){
      url += `&filterByFormula=${encodeURIComponent(filterFormula)}`;
    }
    try{
      const res = await fetchWithTimeout(url, {headers:{Authorization:`Bearer ${pat}`}}, 30000);
      if(!res.ok){
        ...
        if(res.status===422){
          console.warn('422 invalid', tableName, t);
          return null;
        }
        break;
      }
      ...
      if(e.name==='AbortError' && retries<3){ retries++; await new Promise(r=>setTimeout(r, 2000)); continue; }
      if(retries<3){ retries++; await new Promise(r=>setTimeout(r, 1500)); continue; }
      break;
    }
  }while(offset);
  return all;
...
async function fetchAirtableAll(tableName, pageSize=100, fields=null){
  const base=window.AIRTABLE_BASE_ID||localStorage.getItem('effah_api_base')||localStorage.getItem('effah_base_id'); 
  const pat=window.AIRTABLE_PAT||localStorage.getItem('effah_api_pat');
  if(!base||!pat) return [];
  let all=[], offset='';
  ...
  do{
    const url = `https://api.airtable.com/v0/${base}/${encodedTable}?pageSize=${pageSize}${offset?`&offset=${offset}`:''}${fieldsParam}`;
    try{
      const res = await fetchWithTimeout(url, {headers:{Authorization:`Bearer ${pat}`}}, 30000);
      if(!res.ok){
      ...
      if(attempts<3){ attempts++; await new Promise(r=>setTimeout(r, 2000)); continue; }
      break;
    }
  }while(offset);
  return all;
...
function getTripNameForFilter(tripId){
  try{
    ...
    const allTrips = window.allTripUmrahRecords||window.allTripRecords||window.allTrips||[];
    const found = allTrips.find(t=>t.id===tripId);
    if(found && found.fields) return found.fields['Trip'] || found.fields['NAME'] || '';
    const sel=document.getElementById('roomingTripSelect');
...
async function fetchRoomingData(forceReload=false){
  try{
    ...
    if(!tripId){
      ...
      if(cont) cont.innerHTML='<div class="p-8 text-center"><div class="text-[13px] font-bold text-slate-700 mb-1">Sila pilih trip</div><div class="text-[11px] text-slate-400">Pilih trip dari dropdown di atas</div></div>';
    ...
    if(!forceReload && tripId){
      try{
        ...
        if(cached){
          ...
          if(age < 300000 && parsed.jemaah && parsed.rooms && parsed.jemaah.length>0){
            console.log('LOCAL CACHE HIT', Math.round(age/1000)+'s');
            ...
            _roomingIsLoading=false;
            return;
          } else if(parsed.jemaah && parsed.jemaah.length===0){
            localStorage.removeItem('effah_rooming_cache_'+tripId);
          }
        }
      }catch(e){}
    }
    ...
    const base=window.AIRTABLE_BASE_ID||localStorage.getItem('effah_api_base')||localStorage.getItem('effah_base_id'); 
    const pat=window.AIRTABLE_PAT||localStorage.getItem('effah_api_pat');
    if(!base||!pat){ hideRoomingLoading(); _roomingIsLoading=false; return; }

    console.log('FETCH START', tripId);
    ...
    if(!optimizedSuccess || !allJems || allJems.length===0){
      console.warn('FALLBACK 2-step ringan');
      try{
        const [roomsLight, jemsLight, staffLight] = await Promise.all([
          fetchAirtableAll('ROOMING LIST', 100, ['TRIP']),
          fetchAirtableAll('DATA JEMAAH UMRAH', 100, ['TRIP']),
          fetchAirtableAll('STAFF LIST (ROOMING)', 100, ['TRIP'])
        ]);
        console.log('Step1 light:', roomsLight.length, 'rooms,', jemsLight.length, 'jemaah');
        const roomIds = roomsLight.filter(r=>{ const tf=r.fields['TRIP']||[]; return Array.isArray(tf)?tf.includes(tripId):String(tf).includes(tripId); }).map(r=>r.id);
        const jemIds = jemsLight.filter(r=>{ const tf=r.fields['TRIP']||[]; return Array.isArray(tf)?tf.includes(tripId):String(tf).includes(tripId); }).map(r=>r.id);
        const staffIds = staffLight.filter(r=>{ const tf=r.fields['TRIP']||[]; return Array.isArray(tf)?tf.includes(tripId):String(tf).includes(tripId); }).map(r=>r.id);
        console.log('Filtered IDs:', roomIds.length, 'rooms,', jemIds.length, 'jemaah');

        if(jemIds.length>0 || roomIds.length>0){
          const fetchByIds = async (table, ids, fields=null)=>{
            ...
            for(let i=0;i<ids.length;i+=chunkSize){
              ...
              const res = await fetchAirtableWithFilter(table, formula, 100, fields);
          ...
          const essentialJemaah = ['TRIP','NAME','NAMA JEMAAH','NAMA','BOARD BASIS','TRAIN','INSURAN','PAKEJ','STATUS VISA','VISA'];
          const essentialRoom = ['TRIP','NAMA BILIK','LOKASI / CITY','KAPASITI','JEMAAH','JEMAAH TANPA KATIL'];
          const essentialStaff = ['TRIP','NAME','BOARD BASIS','TRAIN','SORT NUMBER','ROOMING LIST'];

          console.log('Step2a essential tanpa attachment');
          const [roomsEss, jemsEss, staffEss] = await Promise.all([
            fetchByIds('ROOMING LIST', roomIds, essentialRoom),
            fetchByIds('DATA JEMAAH UMRAH', jemIds, essentialJemaah),
            fetchByIds('STAFF LIST (ROOMING)', staffIds, essentialStaff)
          ]);
          console.log('Step2a done', roomsEss.length, jemsEss.length);

          allRooms = roomsEss;
          allJems = jemsEss;
          allStaffRaw = staffEss;
          ...
          setTimeout(async ()=>{
            try{
              console.log('Step2b attachments untuk count');
              const attachFields = ['TRIP','PASSPORT COPY','VISA COPY'];
              const jemsAttach = await fetchByIds('DATA JEMAAH UMRAH', jemIds, attachFields);
              const map = new Map(jemsAttach.map(r=>[r.id, r.fields]));
              allRoomingJemaah.forEach(j=>{
                const af = map.get(j.id);
                if(af){
                  if(af['PASSPORT COPY']) j.fields['PASSPORT COPY']=af['PASSPORT COPY'];
                  if(af['VISA COPY']) j.fields['VISA COPY']=af['VISA COPY'];
                }
              });
              try{ updateVisaCountBadge(); }catch(e){}
              console.log('Badge updated');
            }catch(e){ console.warn('Step2b fail', e); }
          }, 500);
    ...
    if(optimizedSuccess){
      allRoomingRecords = allRooms||[];
      allRoomingJemaah = allJems||[];
    } else if(allRooms && allRoomingRecords.length===0){
      allRoomingRecords = allRooms||[];
      allRoomingJemaah = allJems||[];
    }
    ...
    if(allRoomingJemaah.length>0 || allRoomingRecords.length>0){
      ...
      _roomingFirstLoadDone = true;
      try{ localStorage.setItem('effah_rooming_cache_'+tripId, JSON.stringify({_time:_roomingCacheTime, rooms:allRoomingRecords, jemaah:allRoomingJemaah, staff:allStaffRaw})); }catch(e){}
    }
...
function hideRoomingLoading(){
  ...
  if(cont && cont.innerHTML.includes('Sedang memuatkan data jemaah')){
    // Will be overwritten by renderNamelist, but if no data, show empty
    if(allRoomingJemaah.length===0){
      cont.innerHTML='<div class="p-6 text-center text-[11px] text-slate-400">Tiada jemaah untuk trip ini<br><button onclick="fetchRoomingData(true)" class="mt-2 px-3 py-1 bg-[#7A0C2E] text-white rounded-full text-[10px]">Muat Semula</button></div>';
    }
  }
}
...
function populateRoomingTripDropdown(){
  ...
  if(trips.length===0){
    sel.innerHTML='<option value="">Sila pilih trip</option><option value="" disabled>Memuatkan...</option>';
  ...
  if(!currentId){
    ...
    if(cont && !cont.innerHTML.includes('Sila pilih trip')){
      cont.innerHTML='<div class="p-8 text-center"><div class="text-[13px] font-bold">Sila pilih trip</div><div class="text-[11px] text-slate-400">Pilih trip dari dropdown di atas</div></div>';
    }
  }
}
...
function renderNamelist(){
  ...
  try{
    const locUpper = (activeLocation||'MEKAH').toUpperCase();
    for(const rec of allRoomingRecords){
      const jemaahIds = rec.fields['JEMAAH']||[];
      const tanpaIds = rec.fields['JEMAAH TANPA KATIL']||[];
      const recLoc = (rec.fields['LOKASI / CITY']||'MEKAH').toUpperCase();
      for(const id of jemaahIds){
        assignedAnySet.add(id);
        if(recLoc===locUpper) assignedInLocSet.add(id);
      }
      for(const id of tanpaIds){
        assignedAnySet.add(id);
...
function dragJemaah(e,jId){ 
  ...
  try{
    ...
    e.dataTransfer.setData('application/x-jemaah-id',jId);
    e.dataTransfer.effectAllowed='move';
  }catch(err){}
  const r=e.currentTarget; if(r) setTimeout(()=>r.style.opacity='0.3',0); 
  console.log('DRAG JEMAAH', jId);
}
function dragEnd(e){ 
  try{ e.currentTarget.style.opacity='1'; }catch(err){}
...
function dropJemaah(e,roomId){
  e.preventDefault(); 
  e.stopPropagation();
  try{ e.currentTarget.classList.remove('ring-2','ring-[#7A0C2E]/20','drag-over','ring-emerald-300'); }catch(err){}
  document.querySelectorAll('[draggable="true"]').forEach(el=>el.style.opacity='1');
  // If this is a room reorder drag (text/room-id), ignore here - let dropRoomReorder handle it
  try{
    const roomDragId = e.dataTransfer.getData('text/room-id') || window.draggedRoomId || window._draggedRoomId;
  ...
  let jId = '';
  try{
    staffId = e.dataTransfer.getData('text/staff-id') || e.dataTransfer.getData('application/x-staff-id') || window._draggedStaffId || '';
    jId = e.dataTransfer.getData('text/jemaah-id') || e.dataTransfer.getData('application/x-jemaah-id') || e.dataTransfer.getData('text/plain') || window._draggedJemaahId || '';
  }catch(err){
    staffId = window._draggedStaffId || '';
    jId = window._draggedJemaahId || '';
  }
  // If jId is actually a room id and room drag is active, ignore
  try{
    if(jId && jId.startsWith('rec') && allRoomingRecords.some(r=>r.id===jId) && (draggedRoomId || window.draggedRoomId)){
      console.log('dropJemaah ignored - jId is room id', jId);
      return;
    }
  }catch(err){}
  const id=staffId||jId; 
  console.log('dropJemaah attempt', {roomId, staffId, jId, id});
  if(!id) {
    console.warn('dropJemaah no id found', e.dataTransfer.types);
    return;
  }
  const rec=allRoomingRecords.find(r=>r.id===roomId);
  if(rec){
...
async function updateCap(roomId,delta){
  ...
  }catch(e){ console.error(e); alert('Gagal mengemas kini kapasiti bilik: '+e.message); }
}
async function updateRoomField(roomId,field,value,doRender=true){
  ...
  }catch(e){ console.error(e); alert('Gagal mengemas kini data bilik: '+e.message); }
}
...
async function updateJemaahField(jemaahId, field, value){
  ...
  const base=window.AIRTABLE_BASE_ID||localStorage.getItem('effah_api_base')||localStorage.getItem('effah_base_id'); const pat=window.AIRTABLE_PAT||localStorage.getItem('effah_api_pat');
  if(!base||!pat) return alert('Konfigurasi Airtable tidak ditemui. Sila semak tetapan API.');
  const rec=allRoomingJemaah.find(r=>r.id===jemaahId);
...
async function executeCopyRooms(){
  ...
  if(created>0) alert(`Berjaya menyalin ${created} bilik daripada ${src} ke ${activeLocation} (${modeText}).` + (failed>0?` ${failed} bilik gagal disalin.`:''));
  else alert('Gagal menyalin bilik. Sila cuba semula.');
}

function dragStaff(e,staffId){ 
  if(isStaffAssignedInLocation(staffId, activeLocation)) return; 
  ...
  const row=e.currentTarget; if(row) setTimeout(()=>row.style.opacity='0.3',0); 
  console.log('DRAG STAFF', staffId);
}
...
function allowDrop(e){ 
  try { e.preventDefault(); e.stopPropagation(); e.dataTransfer.dropEffect='move'; } catch(err){}
  window._lastDragY=e.clientY; 
  try{ _startAutoScroll(); }catch(err){}
  const el=e.currentTarget;
  if(el && el.classList) el.classList.add('drag-over','ring-2','ring-emerald-300');
}
...
// updateRoomField is already defined above and reused by rooming actions.
...
async function _downloadAllDocs(fieldName, label){
  try{
    ...
    const passNames=['PASSPORT COPY','PASSPORT','PASSPORT_COPY'];
    const searchNames=fieldName.includes('VISA')?visaNames:passNames;
    let withDocs=allRoomingJemaah.filter(j=>{ const a=getFieldAttachments(j.fields||{}, searchNames); return a&&a.length>0; });
    try{
      const locUpper=(activeLocation||'MEKAH').toUpperCase();
      const roomsInLoc=(allRoomingRecords||[]).filter(r=>(r.fields['LOKASI / CITY']||'MEKAH').toUpperCase()===locUpper);
      const ids=new Set(); roomsInLoc.forEach(r=>{ (r.fields['JEMAAH']||[]).forEach(id=>ids.add(id)); });
      if(ids.size>0){ const f=withDocs.filter(j=>ids.has(j.id)); if(f.length>0) withDocs=f; }
    }catch(e){}
    if(withDocs.length===0){ alert(`Tiada ${fieldName} dalam trip ini.`); return; }
    withDocs=withDocs.sort((a,b)=>getJemaahName(a.fields).toUpperCase().localeCompare(getJemaahName(b.fields).toUpperCase()));
    const pdfLib=await loadPdfLib(); const {PDFDocument}=pdfLib; const mergedPdf=await PDFDocument.create();
      ...
      for(let att of atts){
        ...
        try{
          ...
          if(isPdf){
            ...
            let img; if((att.filename||'').toLowerCase().endsWith('.png')) img=await mergedPdf.embedPng(buffer); else img=await mergedPdf.embedJpg(buffer);
            const page=mergedPdf.addPage([595.28, 841.89]); const {width,height}=img.scale(1); const scale=Math.min(595.28/width, 841.89/height)*0.95;
    ...
    const a=document.createElement('a'); a.href=url; a.download=`${label}_${activeLocation||'MEKAH'}.pdf`; a.click(); setTimeout(()=>URL.revokeObjectURL(url), 1000);
...
function updateVisaCountBadge(){
  try{
    ...
    let visaCount=0, passCount=0, visaLoc=0, passLoc=0;
    (allRoomingJemaah||[]).forEach(j=>{
    ...
    try{
      ...
      const roomsInLoc=(allRoomingRecords||[]).filter(r=>(r.fields['LOKASI / CITY']||'MEKAH').toUpperCase()===locUpper);
      const ids=new Set();
      roomsInLoc.forEach(r=>{ (r.fields['JEMAAH']||[]).forEach(id=>ids.add(id)); (r.fields['JEMAAH TANPA KATIL']||[]).forEach(id=>ids.add(id)); });
      const toCheck=ids.size>0?(allRoomingJemaah||[]).filter(j=>ids.has(j.id)):(allRoomingJemaah||[]);
      toCheck.forEach(j=>{
        if(getFieldAttachments(j.fields||{}, visaNames)) visaLoc++;
        if(getFieldAttachments(j.fields||{}, passNames)) passLoc++;
      });
    }catch(e){ visaLoc=visaCount; passLoc=passCount; }
    console.log(`Badge - Total:${allRoomingJemaah?.length} Visa:${visaCount} Pass:${passCount} | Loc ${activeLocation} Visa:${visaLoc} Pass:${passLoc}`);
    const vBadge=document.getElementById('visaCountBadge');
    const pBadge=document.getElementById('passportCountBadge');
    if(vBadge) vBadge.textContent=visaLoc;
    if(pBadge) pBadge.textContent=passLoc;
  }catch(e){ console.error(e); }
}
