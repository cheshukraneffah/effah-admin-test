...
// ROOMING V103.37 PROXY FULL - FIX 403 + FIELD MISMATCH
// Base: appSsn4JyQD4DnYu0 | Proxy: https://effah-proxy.cheshukran-effah.workers.dev/api
console.log('ROOMING V103.37 PROXY FULL loaded');
...
// --- PROXY HELPERS (NO PAT, NO fields[] filter) ---
async function effahProxyFetch(url, opts={}){
  // opts already without Authorization
  const res = await fetch(url, opts);
  const txt = await res.text();
  try { return JSON.parse(txt); } catch(e){ 
    if(!res.ok) throw new Error(`Proxy ${res.status}: ${txt.slice(0,500)}`);
    return {};
  }
}
...
// Map old URL encoded names to TABLE IDs for quick replace
...
async function loadStaffList(){
  try{
    ...
    try{
      ...
      allStaff = await effahGetAll(EFFAH_T.STAFF, filter);
    }catch(e){
      console.warn('proxy staff fetch failed, fallback local', e);
      staffList=JSON.parse(localStorage.getItem(getStaffStorageKey())||'[]'); renderStaffList(); return;
    }
    ...
    console.error('loadStaffList proxy failed', e);
...
async function fetchRoomingData(forceReload=false){
  try{
    ...
    try{
      do{ 
        const res=await fetch(`${EFFAH_PROXY}?base=${EFFAH_BASE}&table=${EFFAH_T.ROOMING}?pageSize=100${offset?`&offset=${offset}`:''}`); 
    ...
    try{
      do{ 
        const res=await fetch(`${EFFAH_PROXY}?base=${EFFAH_BASE}&table=${EFFAH_T.PAX}?pageSize=100${offset?`&offset=${offset}`:''}`); 
        if(!res.ok){ 
          const txt=await res.text();
  ...
  window.updateJemaahField = async function(jemaahId, field, value){
    ...
    if(field==='STATUS VISA'){
      ...
      if(v==='' || v.toUpperCase()==='- VISA' || v.toUpperCase()==='VISA' || v==='-' || v==='--'){
        ...
        if(typeof renderNamelist==='function') renderNamelist();
        try{
          const res = await fetch(`${EFFAH_PROXY}?base=${EFFAH_BASE}&table=${EFFAH_T.PAX}&recordId=${jemaahId}`,{
            method:'PATCH',
