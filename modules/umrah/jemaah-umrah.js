...
function updateRoomField(roomId, field, value, shouldRender=true){
  const rec = allRoomingRecords.find(r=>r.id===roomId);
  if(rec && rec.fields) rec.fields[field] = value;
  if(shouldRender){
    renderRoomingGrid();
    renderStaffList();
    renderNamelist();
...
// The active updateRoomField implementation above already handles these fields.
