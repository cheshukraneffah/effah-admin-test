...
function renderRoomingHTML(){
  ...
  <div class="flex flex-col gap-2.5 p-2">
    <div class="bg-white rounded-2xl border border-slate-200 p-2.5 flex flex-wrap items-center justify-between gap-2">
      ...
      <div class="flex items-center gap-1.5 text-[11px]">
        ...
        <button onclick="window.fetchRoomingData || fetchRoomingData(true)" title="Reload data dari Airtable" class="w-6 h-6 rounded-full border bg-white hover:bg-slate-50 text-[10px]"><i class="fa-solid fa-rotate"></i></button>
      </div>
    </div>
  ...
  setTimeout(()=>window.fetchRoomingData || fetchRoomingData(), 100);
}
...
function hideRoomingLoading(){
  ...
  if(cont && cont.innerHTML.includes('Memuatkan jemaah')){
    ...
    if(allRoomingJemaah.length===0){
      cont.innerHTML='<div class="p-6 text-center text-[11px] text-slate-400">Tiada jemaah untuk trip ini<br><button onclick="window.fetchRoomingData || fetchRoomingData(true)" class="mt-2 px-3 py-1 bg-[#7A0C2E] text-white rounded-full text-[10px]">Retry Load</button></div>';
    }
  }
}
...
function createMissingRoomingStructure(){
  ...
  if(!hasNamelist || !hasGrid || modul.innerHTML.trim().length<100){
      ...
      <div id="roomingHeader" class="p-4 border-b bg-white">
        <div class="flex justify-between items-center">
          ...
          <div class="flex gap-2">
            ...
            <button onclick="window.fetchRoomingData || fetchRoomingData(true)" class="text-[11px] bg-[#7A0C2E] text-white px-3 py-1 rounded-full">Reload</button>
          </div>
        </div>
    ...
    `;
    setTimeout(()=>{ if(typeof populateRoomingTripDropdown==='function') populateRoomingTripDropdown(); if(typeof window.fetchRoomingData || fetchRoomingData==='function') window.fetchRoomingData || fetchRoomingData(); }, 500);
    return true;
  }
  return false;
}
