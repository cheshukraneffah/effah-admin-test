...
function renderTripSidebarList(records){
  ...
  if(tripFilters.month !== 'All'){
    filtered = filtered.filter(rec=> getMonthKeyLong(rec.fields['Mula Pakej']).toUpperCase().includes(tripFilters.month.toUpperCase()));
  }
  if(tripFilters.airline !== 'All'){
    filtered = filtered.filter(rec=> (rec.fields['Penerbangan']||'').toUpperCase() === tripFilters.airline.toUpperCase());
  }
  if(tripFilters.tempoh !== 'All'){
    filtered = filtered.filter(rec=> (rec.fields['Tempoh Pakej']||'').toUpperCase() === tripFilters.tempoh.toUpperCase());
  }
  if(tripFilters.musim !== 'All'){
    filtered = filtered.filter(rec=> (rec.fields['Musim']||'').toUpperCase() === tripFilters.musim.toUpperCase());
  }
  filtered.sort((a,b)=>{
    ...
    trips.forEach(rec=>{
      ...
      card.innerHTML = `
        <div class="overflow-hidden">
          <h4 class="font-bold text-[12px] text-slate-900 leading-snug break-words line-clamp-2">${displayTitle}</h4>
          <div class="flex flex-wrap gap-1 mt-2">
            <span class="${statusBadge.cls} border text-[9px] font-bold px-1.5 py-0.5 rounded-full flex items-center gap-1 whitespace-nowrap"><span class="w-1.5 h-1.5 rounded-full ${statusBadge.dot}"></span>${statusBadge.label}</span>
            ${tempoh ? `<span class="bg-slate-50 text-slate-600 border border-slate-200 text-[9px] font-bold px-1.5 py-0.5 rounded-full whitespace-nowrap">${tempoh}</span>` : ''}
            <span class="${airlineStyle.bg} ${airlineStyle.text} ${airlineStyle.border} border text-[9px] font-bold px-1.5 py-0.5 rounded-full whitespace-nowrap">${airline}</span>
          </div>
          <div class="flex items-center justify-between mt-2 pt-2 border-t border-dashed border-slate-100">
            <span class="text-[10px] text-slate-500 font-medium truncate mr-2">${sektor}</span>
            <span class="shrink-0">${bottomInfo}</span>
          </div>
        </div>
      `;
      container.appendChild(card);
...
function setAdvancedFilter(type, val){
  tripFilters[type] = val;
  renderTripSidebarList(allTripUmrahRecords);
}
function clearAdvancedFilters(){
  tripFilters = { hijri: 'All', search: '', month: 'All', airline: 'All', tempoh: 'All', musim: 'All' };
  const s=document.getElementById('searchTripSidebar'); if(s) s.value='';
  renderTripSidebarList(allTripUmrahRecords);
}
function updateAdvancedFilterOptions(allRecords){
  const monthSel=document.getElementById('filterMonth');
  const airlineSel=document.getElementById('filterAirline');
  const tempohSel=document.getElementById('filterTempoh');
  const musimSel=document.getElementById('filterMusim');
  if(!monthSel) return;
  
  // Month options from data
  const months={};
  allRecords.forEach(r=>{ const k=getMonthKeyLong(r.fields['Mula Pakej']); if(k!=='TBC') months[k]=true; });
  const monthKeys=Object.keys(months).sort((a,b)=> new Date(allRecords.find(x=>getMonthKeyLong(x.fields['Mula Pakej'])===a)?.fields['Mula Pakej']||0) - new Date(allRecords.find(x=>getMonthKeyLong(x.fields['Mula Pakej'])===b)?.fields['Mula Pakej']||0));
  const curMonth=tripFilters.month;
  monthSel.innerHTML='<option value="All">Bulan: Semua</option>'+monthKeys.map(k=>`<option value="${k}" ${curMonth===k?'selected':''}>${k}</option>`).join('');
  
  // Airline
  const airlines={};
  allRecords.forEach(r=>{ const a=(r.fields['Penerbangan']||'').trim(); if(a) airlines[a]=true; });
  const curAir=tripFilters.airline;
  airlineSel.innerHTML='<option value="All">Airline: Semua</option>'+Object.keys(airlines).sort().map(a=>`<option value="${a}" ${curAir===a?'selected':''}>${a}</option>`).join('');
  
  // Tempoh
  const tempohs={};
  allRecords.forEach(r=>{ const t=(r.fields['Tempoh Pakej']||'').trim(); if(t) tempohs[t]=true; });
  const curTem=tripFilters.tempoh;
  tempohSel.innerHTML='<option value="All">Tempoh: Semua</option>'+Object.keys(tempohs).sort().map(t=>`<option value="${t}" ${curTem===t?'selected':''}>${t}</option>`).join('');
  
  // Musim
  const musims={};
  allRecords.forEach(r=>{ const m=(r.fields['Musim']||'').trim(); if(m) musims[m]=true; });
  const curMus=tripFilters.musim;
  musimSel.innerHTML='<option value="All">Musim: Semua</option>'+Object.keys(musims).sort().map(m=>`<option value="${m}" ${curMus===m?'selected':''}>${m}</option>`).join('');
}
...
function updateHijriFilterTabs(allRecords){
  ...
  updateAdvancedFilterOptions(allRecords);
...
async function updateAirtableField(recId, fieldName, value){
  ...
  try{
    ...
    if(response.ok){
      ...
      if((errData.error?.message||'').includes('Insufficient permissions to create new select option')){ alert('Option "'+fieldsData[fieldName]+'" belum ada di Airtable field '+fieldName+'. Sila tambah manual di Airtable dulu.'); } else { alert('Gagal mengemaskini field di Airtable: ' + (errData.error?.message||'')); }
