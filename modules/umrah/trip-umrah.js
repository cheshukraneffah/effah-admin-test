...
function sortTripJemaahBy(field){ if(tripJemaahSortField===field){ tripJemaahSortDir=tripJemaahSortDir==='asc'?'desc':'asc'; }else{ tripJemaahSortField=field; tripJemaahSortDir='asc'; } currentTripJemaahList=sortJemaahArray(currentTripJemaahList, field, tripJemaahSortDir); const tbody=document.getElementById('tripJemaahTableBody'); if(tbody){ tbody.innerHTML=currentTripJemaahList.map((j,idx)=>{ const jf=j.fields; return `<tr><td class='p-3 text-center'>${idx+1}</td><td class='p-3 font-bold'>${jf['NAME']||'-'}</td><td class='p-3'>${jf['PASSPORT NO.']||'-'}</td></tr>`; }).join(''); } }
